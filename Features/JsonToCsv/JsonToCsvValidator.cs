...
{
    public static Result<List<Dictionary<string, string>>> Validate(JsonToCsvRequest? request)
    {
        if (request is null)
        ...
        if (request.Separator is { Length: not 1 })
            return Result<List<Dictionary<string, string>>>.Failure("Separator debe ser un único carácter.");

        if (request.Encoding is { Length: > 0 } && !EncodingHelper.TryGetEncoding(request.Encoding, out _))
            return Result<List<Dictionary<string, string>>>.Failure($"Encoding no válido: {request.Encoding}");

        if (request.NewLine is { Length: > 0 } && request.NewLine is not "\n" and not "\r\n")
            return Result<List<Dictionary<string, string>>>.Failure("NewLine debe ser \"\\n\" o \"\\r\\n\".");
