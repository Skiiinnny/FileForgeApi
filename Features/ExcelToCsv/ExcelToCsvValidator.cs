...
public static class ExcelToCsvValidator
{
    public static Result<(byte[]? FileBytes, bool UseUrl)> Validate(ExcelToCsvRequest? request)
    {
        if (request is null)
        ...
        if (request.Encoding is { Length: > 0 } && !EncodingHelper.TryGetEncoding(request.Encoding, out _))
            return Result<(byte[]?, bool)>.Failure($"Encoding no válido: {request.Encoding}");

        if (request.NewLine is { Length: > 0 } && request.NewLine is not "\n" and not "\r\n")
            return Result<(byte[]?, bool)>.Failure("NewLine debe ser \"\\n\" o \"\\r\\n\".");
