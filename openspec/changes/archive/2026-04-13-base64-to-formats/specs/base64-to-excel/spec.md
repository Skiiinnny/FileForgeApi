...
El sistema SHALL aceptar un objeto JSON con `base64Content` (cadena Base64 obligatoria) y un parámetro opcional `filename`, y SHALL retornar el contenido decodificado como archivo Excel con cabeceras `Content-Type: application/vnd.openxmlformats-officedocument.spreadsheetml.sheet` y `Content-Disposition: attachment; filename=<filename>`.
...
- **WHEN** se envía `POST /api/base64/to-excel` con `{ "base64Content": "" }` o sin el campo
